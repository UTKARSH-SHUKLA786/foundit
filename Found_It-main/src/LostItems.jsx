  ...
  return (
    <div className="min-h-screen text-white p-8 relative">
      ...
      {selectedImg && (
        ...
        >
          <button className="absolute top-10 right-10 text-white text-5xl font-light hover:text-blue-400 transition-colors">&times;</button>
          <img 
            src={selectedImg} 
            alt="Enlarged" 
            className="max-w-full max-h-[85vh] rounded-2xl shadow-[0_0_50px_rgba(59,130,246,0.3)] border border-white/10 transition-transform scale-100"
      ...
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 lg:grid-cols-3 gap-6">
        {items.map((item) => (
          <div key={item._id} className="bg-gray-900/50 backdrop-blur-md border border-blue-500/20 p-6 rounded-3xl relative hover:border-blue-500/50 transition-all flex flex-col">
            ...
            >
              {item.image ? (
                ...
                <div className="flex flex-col items-center opacity-20">
                  ...
                  <p className="text-[10px] mt-1 uppercase font-bold text-blue-400">No Image</p>
                </div>
              )}
            </div>
            ...
            <div className="pt-4 border-t border-white/5 space-y-3 mt-auto">
              <div className="flex justify-between items-end">
                <div>
                  <span className="text-[11px] text-gray-500 uppercase font-bold block tracking-widest">Contact Owner</span>
              ...
              <div className="flex justify-between items-center pt-2 border-t border-white/5">
                <p className="text-[12px] text-gray-500">By: <span className="text-blue-400/80 font-mono">{item.reportedBy}</span></p>
                {item.reportedBy === currentUser && (
                  <span className="text-[8px] bg-blue-500 text-white px-2 py-1 rounded font-black uppercase tracking-tighter">Your Post</span>
                )}
              </div>
            </div>
          </div>
